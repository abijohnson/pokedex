...
export default {
    ...
    async mounted() {
        if(this.pkmnStore.listOfPokemonByGeneration[9].length === 0){
            await this.pkmnStore.getPokemonByGeneration(9)
        }
        this.listOfPokemon = this.pkmnStore.listOfPokemonByGeneration[9]
    ...
    methods: {
        ...
        async getLocationsByPokemon(pokemon) {
            await this.pkmnStore.getLocationsByPokemon(9, pokemon)
        }
    }
}
</script>
