...
export default {
    ...
    async mounted() {
        if(this.pkmnStore.listOfPokemonByGeneration[4].length === 0){
            await this.pkmnStore.getPokemonByGeneration(4)
        }
        this.listOfPokemon = this.pkmnStore.listOfPokemonByGeneration[4]
    ...
    methods: {
        ...
        async getLocationsByPokemon(pokemon) {
            await this.pkmnStore.getLocationsByPokemon(4, pokemon)
        }
    }
}
</script>
