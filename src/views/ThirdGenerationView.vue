...
export default {
    ...
    async mounted() {
        if(this.pkmnStore.listOfPokemonByGeneration[3].length === 0){
            await this.pkmnStore.getPokemonByGeneration(3)
        }
        this.listOfPokemon = this.pkmnStore.listOfPokemonByGeneration[3]
    ...
    methods: {
        ...
        async getLocationsByPokemon(pokemon) {
            await this.pkmnStore.getLocationsByPokemon(3, pokemon)
        }
    }
}
</script>
