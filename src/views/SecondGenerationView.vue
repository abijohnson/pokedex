...
export default {
    ...
    async mounted() {
        if(this.pkmnStore.listOfPokemonByGeneration[2].length === 0){
            await this.pkmnStore.getPokemonByGeneration(2)
        }
        this.listOfPokemon = this.pkmnStore.listOfPokemonByGeneration[2]
    ...
    methods: {
        ...
        async getLocationsByPokemon(pokemon) {
            await this.pkmnStore.getLocationsByPokemon(2, pokemon)
        }
    }
}
</script>
