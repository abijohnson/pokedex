...
export default {
    ...
    async mounted() {
        if(this.pkmnStore.listOfPokemonByGeneration[7].length === 0){
            await this.pkmnStore.getPokemonByGeneration(7)
        }
        this.listOfPokemon = this.pkmnStore.listOfPokemonByGeneration[7]
    ...
    methods: {
        ...
        async getLocationsByPokemon(pokemon) {
            await this.pkmnStore.getLocationsByPokemon(7, pokemon)
        }
    }
}
</script>
