                                ...
                                <div class="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl">
                                        ...
                                        <div class="flex items-start justify-between">
                                            <DialogTitle>
                                                ...
                                                <p class="text-base pt-1 text-gray-500">
                                                    <span v-if="pokemon.id > 99" class="inline-flex">
                                                        #
                                                    </span>
                                                    <span v-else-if="pokemon.id > 9" class="inline-flex">
                                                        #0
                                                    </span>
                                                    <span v-else class="inline-flex">
                                                        #00
                                                    </span>
                                                    <span class="inline-flex">{{ pokemon.id }}</span>
                                                </p>
                                            </DialogTitle>
                                    ...
                                    <div class="relative mt-6 flex-1 px-4 sm:px-6">
                                        ...
                                        <div class="grid grid-cols-2 px-10 py-7">
                                            <img :src="pokemon.sprite" class="mx-auto w-3/4" />
                                            <img :src="pokemon.spriteBack" class="mx-auto w-3/4" />
                                        </div>
                                        <!--Tabs-->
                                        <div>
                                            <div class="sm:hidden">
                                            ...
                                            <div class="hidden sm:block">
                                                <div class="border-b border-gray-200">
                                                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                                                        <p v-for="tab in tabs" :key="tab.name" @click="currentTab = tab" :class="[isCurrentTab(tab) ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700', 'cursor-pointer group inline-flex items-center border-b-2 py-4 px-1 text-sm font-medium']" :aria-current="tab.current ? 'page' : undefined">
                                        ...
                                        <div>
                                            <component
                                                ...
                                                @getLocations="$emit('getLocations' , pokemon)"
                                            />
                                        </div>
                                    </div>
                                </div>
...
export const tabs = [
    { name: 'About', icon: InformationCircleIcon, component: shallowRef(About) },
    { name: 'Locations', icon: HomeIcon, component: shallowRef(Locations) },
    { name: 'Base stats', icon: BoltIcon, component: shallowRef(BaseStats) },
]
export default {
    components: {
